...
int main()
{
    for (auto N : { 1, 10, 100, 1'000, 10'000, 100'000, 1'000'000, 10'000'000, 100'000'000 }) {

        std::vector<int16_t> numbers;
        numbers.reserve(N);
        std::generate_n(std::back_inserter(numbers), N,
            std::bind(std::uniform_int_distribution<int16_t>(), std::default_random_engine()));
        ...
        std::cout << std::setw(10) << N << " elements | " << std::fixed << "seq: " << durationSeq.count() << " s "
